...
for (i in 1:nrow(t)) {
  ...
  v1plot <- ggplot(v[[i]],aes(opkomstratio,blancoratio,col=big)) + geom_point() + geom_smooth(method="lm")
  v2plot <- ggplot(v[[i]],aes(ingeschreven,foertratio)) + 
    geom_point() + 
...
In het gebruikte [log-log model](https://nl.wikipedia.org/wiki/Machtsfunctie) wordt voor de verkiezingen van 2012, `r t[1]$model`% van de variatie in de foertratio verklaard door de het aantal kiesgerechtigden. Zoals je kan zien in bovenstaande grafiek groeit de foertratio sterk bij steden met tussen de 0 en 40.000 kiesgerechtigden en zwakt de stijging daarna af. `r t[1]$model`%, dat is vrij veel. Maar dat wil zeggen dat de overige `r 1 - t[1]$model`% verklaard wordt door een heleboel andere factoren. Een uitbreiding van het model met andere factoren zoals inkomen, leeftijd en migratie zou voor interessante inzichten kunnen zorgen.

Dit verband is voor elk (gemeenteraads)verkiezingsjaar aanwezig en significant. 